...
public class CreateEmployeeCommandHandler : IRequestHandler<CreateEmployeeCommand, Result<EmployeeDto>>
{
    ...
    public async Task<Result<EmployeeDto>> Handle(CreateEmployeeCommand request, CancellationToken cancellationToken)
    {
        ...
        var employee = new Employee
        {
            ...
            Email = request.Email, Phone = request.Phone, Mobile = request.Mobile,
            DateOfBirth = request.DateOfBirth, Gender = request.Gender, MaritalStatus = request.MaritalStatus,
            NationalId = request.NationalId, Nationality = request.Nationality,
            Address = request.Address, City = request.City, Country = request.Country,
            HireDate = request.HireDate, ContractType = request.ContractType,
            DepartmentId = request.DepartmentId, PositionId = request.PositionId,
            BranchId = request.BranchId, ManagerId = request.ManagerId, BasicSalary = request.BasicSalary
        };

        await _uow.Repository<Employee>().AddAsync(employee, cancellationToken);
        await _uow.SaveChangesAsync(cancellationToken);
        await _auditService.LogAsync(HRMS.Domain.Enums.AuditAction.Create, nameof(Employee), employee.Id.ToString(), cancellationToken: cancellationToken);
